...
class TestBarChart < Minitest::Test
  ...
  def test_cat_axis_position_for_horizontal_bar_chart
    axes = @chart.axes
    assert_equal(:l, axes[:cat_axis].ax_pos, "cat_axis.ax_pos must be :l by default for horizontal bar charts")
  end

  def test_val_axis_position_for_vertical_bar_chart
    @chart.bar_dir = :col
    axes = @chart.axes
    assert_equal(:l, axes[:val_axis].ax_pos, "val_axis.ax_pos must be :l for vertical bar charts (bar_dir = :col)")
  end
end
