...
class TestSimpleTypedList < Test::Unit::TestCase
  ...
  def test_concat_assignment
    assert_raise(ArgumentError) { @list.concat([1, nil]) }
    assert_nothing_raised { @list.concat [1] }
  end

  def test_concat_should_return_self
  ...
  def test_push_assignment
    assert_raise(ArgumentError) { @list.push("1") }
    assert_raise(ArgumentError) { @list.push(1, nil) }
    assert_nothing_raised { @list.push(1, 2) }
  end

  def test_push_should_return_self
    assert_equal(@list, @list.push(1))
  end

  def test_push_should_mutate_object
    @list.push(1)
